...
**Whitelist:** ${formatNumber(meta.stats.database.whitelists)}`, 
    ...
    async checkTempRoles () {
        logError('checkTempRoles checkTempRoles checkTempRolescheckTempRoles')
        ...
        GlobalData.tempRoles.filter(t => t.expire <= Date.now()).forEach(async t => {
            try {
                ...
                if (member) {
                    logOk(`Revoking role \`${t.role}\` from \`${t.id}\``);
                    if(t.role === 'muted'){
                ...
                logWarn(`Revoking role \`${t.role}\` from \`${t.id}\` probably failed`);
                dir(error)
            }
        })
    }
