...
module.exports = ShowYourWorkHandler = async (message, txChungus) =>{
    ...
    //Checking rules
    if(lines.length > 5 || links.length !== 1){
        return replyDelete(message, `**⚠️ Your message will be deleted in 15 seconds! ⚠️**
Channel usage rules:
- Messages MUST have exactly ONE link (ex: image, forum post, github, discord invite).
- Messages CANNOT be over 5 lines.`);
    }
