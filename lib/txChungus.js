...
module.exports = class txChungus {
    ...
    async setupWatchers () {
        ...
        cmdWatcher.on('change', file => {
            this.setCommand(path.basename(file));
    ...
    async messageHandler (message) {
        ...
        message.txIsAdmin = this.config.commands.admins.includes(message.author.id);
        ...
        if(!message.txIsAdmin && doesMessageContains(message.content, GlobalData.malwareStrings)){
            ...
            if(!this.recentInfectedWarnings.includes(message.author.id)){
                ...
                try {
                    ...
                    await GlobalActions.tmpRoleAdd('muted', message.author.id, expiration, 'malware_infection');
                } catch (error) {
                    dir(error)
                }
            }
