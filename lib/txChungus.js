...
module.exports = class txChungus {
    constructor(config, profile) {
        ...
        this.malwareStringsFile = `./data/malwareStrings_${GlobalData.profile}.txt`;
    ...
    async setupData (malwareStringsOnly=false) {
        try {
            ...
            if (!malwareStringsRaw.length) {
                ...
                dir(GlobalData.malwareStrings); //DEBUG
            }
    ...
    async messageHandler (message) {
        ...
        if(doesMessageContains(message.content, GlobalData.malwareStrings)){
        ...
        if(doesMessageContains(message.content, this.config.autoMuteStrings)){
            if(message.channel.type === 'dm' || message.deleted) return;
            ...
            try {
                const expiration = Date.now() + 10080 * 60e3;
                await GlobalActions.tmpRoleAdd('muted', message.author.id, expiration, 'posting suspect message or pinging everyone');
