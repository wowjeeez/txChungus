...
module.exports = GeneralHandler = async (message, txChungus) => {
    ...
    if (txChungus.config.bannedStrings.filter((w) => message.content.toLowerCase().includes(w)).length) {
        logError(`${message.author.tag} posted a malware:`);
        logWarn(message.content);
        message.delete().catch(() => { });
        return message.reply(`my dude, you have likely been infected by a malware.`);
    }
